...
import { OPENCLAW_AGENTS_DIR, OPENCLAW_CONFIG_PATH } from '../lib/openclaw-paths.js';
 ...
 * Update agent model via Gateway config.patch (supports hot reload)
 */
router.patch('/', async (req, res) => {
  try {
    ...
    if (!agentId || !model) {
      return res.status(400).json({ ok: false, error: "Missing agentId or model" });
    }

    const snapshot = await getConfigSnapshot();
    if (snapshot?.valid === false || !isPlainObject(snapshot?.config)) {
      return res.status(400).json({ ok: false, error: "Gateway config is invalid or unavailable" });
    }

    const baseHash = resolveConfigSnapshotHash(snapshot);
    if (!baseHash) {
      return res.status(500).json({ ok: false, error: "Missing baseHash from config snapshot" });
    }

    const config = snapshot.config;
    const agentEntry = findAgentConfigEntry(config, agentId);
    if (!agentEntry) {
      return res.status(404).json({ ok: false, error: `Agent not found in agents.list: ${agentId}` });
    }

    const knownModels = collectKnownModels(config);
    if (!knownModels.has(model)) {
      return res.status(400).json({ ok: false, error: `Unknown model: ${model}` });
    }
    ...
    return res.status(statusForError(error)).json({ ok: false, error });
  }
});

/**
 * GET /api/config/agent-model
 * Check if Gateway is available for hot reload
 */
